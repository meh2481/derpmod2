/*

 TITLE_TEXT.C

 Map Source File.

 Info:
   Section       :
   Bank          : 0
   Map size      : 18 x 3
   Tile set      : title_bg.gbr
   Plane count   : 2 planes (16 bits)
   Plane order   : Planes are continues
   Tile offset   : 0
   Split data    : No

 This file was generated by GBMB v1.8

*/
#include <gb/gb.h>
#pragma bank 255
BANKREF(title_text)

#define title_textWidth 18
#define title_textHeight 3
#define title_textBank 0

#define title_text title_textPLN0
const unsigned char title_textPLN0[] =
{
  0x2B,0x2C,0x2D,0x2E,0x2F,0x30,0x31,0x32,0x33,0x34,
  0x35,0x36,0x37,0x38,0x39,0x3A,0x3B,0x3C,0x3D,0x3E,
  0x3F,0x40,0x41,0x42,0x43,0x44,0x45,0x46,0x47,0x48,
  0x49,0x4A,0x4B,0x4C,0x4D,0x4E,0x4F,0x50,0x51,0x52,
  0x53,0x54,0x55,0x56,0x57,0x58,0x59,0x5A,0x5B,0x5C,
  0x5D,0x5E,0x5F,0x60
};

const unsigned char title_textPLN1[] =
{
  0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,
  0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,
  0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,
  0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,
  0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,0x09,
  0x09,0x09,0x09,0x09
};

void show_title_tile(uint8_t tile) BANKED {
  VBK_REG = VBK_TILES;
  set_bkg_tiles(1 + tile % title_textWidth, 1+tile / title_textWidth, 1, 1, &title_text[tile]);
  VBK_REG = VBK_ATTRIBUTES;
  set_bkg_tiles(1 + tile % title_textWidth, 1+tile / title_textWidth, 1, 1, &title_textPLN1[tile]);
  VBK_REG = VBK_TILES;
}

/* End of TITLE_TEXT.C */
