...
const unsigned char bkg_tile_attribs[] = {
  // Use palette 1
...
const palette_color_t * const spritePaletteData[] = {
  // Earth palette
  RGB(0x00,0x00,0x00), /* Black */
  RGB8(0x71,0x71,0xFF), /* Blue */
  RGB8(0x00,0x64,0x00), /* Green */
  RGB(0x00,0x00,0x00), /* Black */
};

const palette_color_t * const backgroundPaletteData[] = {
  // Background palette for frame
  RGB8(0xFF,0xFF,0xFF), /* White */
  RGB8(0xFF,0xFF,0x00), /* Yellow */
  RGB8(0x2E,0x2E,0x2E), /* Dark Grey */
  RGB(0x00,0x00,0x00), /* Black */
  // Background palette for stars
  RGB8(0xFF,0x91,0x91), /* Bright Red */
  RGB8(0xFF,0xFF,0xFF), /* White */
  RGB8(0xD7,0x43,0x15), /* Dark Red */
  RGB(0x00,0x00,0x00), /* Black */
  // Background palette for door
  RGB8(0x00,0x00,0x00), /* Black */
  RGB8(0xFF,0xFF,0xFF), /* White */
  RGB8(0x5C,0x5C,0x5C), /* Light Grey */
  RGB8(0x20,0x20,0x20), /* Very Dark Grey */
};

const unsigned char door_tile_attribs[] = {
  // Use palette 2
...
void scroll(void)
{
  ...
  if (sposx.b.h <= 0x00) {
    sposx.b.h = 0x00;
    /* Invert speed */
    sspx.w = -(int16_t)sspx.w;
  } else if(sposx.b.h >= SCREEN_WIDTH - 8) {
    sposx.b.h = SCREEN_WIDTH - 8;
    /* Invert speed */
    sspx.w = -(int16_t)sspx.w;
  }
  /* Y position */
  if(sposy.b.h <= SCREEN_Y_OFFSET) {
    sposy.b.h = 0x00;
    /* Invert speed */
    sspy.w = -(int16_t)sspy.w;
  } else if(sposy.b.h >= SCREEN_HEIGHT + SCREEN_Y_OFFSET) {
    sposy.b.h = SCREEN_HEIGHT + SCREEN_Y_OFFSET;
    /* Invert speed */
    sspy.w = -(int16_t)sspy.w;
  }
  place_sprite();
...
static uint8_t bSwitchedSound = 0x0;
...
void main(void)
{
  ...
  set_bkg_palette(0, 3, backgroundPaletteData);  // 3 bg palettes
  ...
  for(i = 0; i < 32; i+=8) {
    for(j = 0; j < 32; j+=8) {
      ...
      // Switch over to attributes to set the palette for the background tiles
      VBK_REG = VBK_ATTRIBUTES;
      set_bkg_tiles(i, j, 8, 8, bkg_tile_attribs);
  ...
  // Set door tiles to use palette 2
  VBK_REG = VBK_ATTRIBUTES;
  for(i = 0; i < 12; i++) {
    set_win_tiles(2+i, 2, 1, 6, door_tile_attribs);
  }
  VBK_REG = VBK_TILES;
